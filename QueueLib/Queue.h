...
public:
    ...
    //+ задания
    inline int min_elem(); //Поиск минимального э.т
    inline int max_elem(); //Поиск максимального эл.т.
...
{
    //Если мест нет, записываем полученный элемент вместо первого, 
    //номер первого элемента сдвигаем на вперед (первый элемент теперь последний), тк кольцо
    if (count >= length)
    {
        ind++;
        count--;
